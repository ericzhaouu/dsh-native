...
type LifecycleSdk = Pick<Sdk, "setActiveEmbeddedRun" | "clearActiveEmbeddedRun" |
  ...
  "runAgentHarnessAgentEndHook" | "runAgentHarnessBeforeAgentFinalizeHook">;
...
export function createNativeHarness(
  ...
  async function runAttempt(p: Attempt): Promise<Result> {
    ...
    let hookContext: Parameters<LifecycleSdk["runAgentHarnessAgentEndHook"]>[0]["ctx"] | undefined;
    ...
    const result: Result = {
      ...
      lastAssistant: assistant, currentAttemptAssistant: assistant,
    ...
    if (sdk && hookContext) {
      try {
        sdk.runAgentHarnessAgentEndHook({
          ctx: hookContext, event: {
