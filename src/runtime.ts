...
async function executeChild(
  ...
  const peer = new JsonRpcPeer(child.stdout, child.stdin, {
    ...
    async onRequest(method, params) {
      ...
      const task = input.executeTool(call, AbortSignal.any([input.signal, controller.signal]));
      toolTasks.add(task);
      try { return await task; }
      finally { toolTasks.delete(task); toolControllers.delete(call.callId); }
    },
  });
  const abort = (): void => {
  ...
  try {
    ...
    await timeout(Promise.race([
      ready,
      peer.closed.then(() => { throw new Error(`DSH stopped before bridge initialization: ${stderr}`); }),
